// Classe TouchTexture (Textura de Toque)
class TouchTexture {
  constructor() {
    this.size = 64;
    this.width = this.height = this.size;
    this.maxAge = 64;
    this.radius = 0.25 * this.size; // Raio de toque muito maior para um efeito mais óbvio
  ...
  update() {
    ...
    // Usa iteração reversa para remover itens com segurança
  ...
  addTouch(point) {
    ...
    if (last) {
      ...
      vx = dx / d;
      vy = dy / d;
      force = Math.min(dd * 20000, 2.0); // Força muito mais forte para um efeito bem perceptível
    }
...
// Classe GradientBackground (Fundo em Gradiente)
class GradientBackground {
  constructor(sceneManager) {
    ...
    this.uniforms = {
      ...
      uColor1: { value: new THREE.Vector3(0.945, 0.353, 0.133) }, // F15A22 - Laranja
      uColor2: { value: new THREE.Vector3(0.039, 0.055, 0.153) }, // 0a0e27 - Azul Marinho
      uColor3: { value: new THREE.Vector3(0.945, 0.353, 0.133) }, // F15A22 - Laranja
      uColor4: { value: new THREE.Vector3(0.039, 0.055, 0.153) }, // 0a0e27 - Azul Marinho
      uColor5: { value: new THREE.Vector3(0.945, 0.353, 0.133) }, // F15A22 - Laranja
      uColor6: { value: new THREE.Vector3(0.039, 0.055, 0.153) }, // 0a0e27 - Azul Marinho
      uSpeed: { value: 1.2 }, // Animação mais rápida
      uIntensity: { value: 1.8 },
      uTouchTexture: { value: null },
      uGrainIntensity: { value: 0.08 },
      uZoom: { value: 1.0 }, // Controle de zoom/escala - menor = menos zoom (mais visível)
      uDarkNavy: { value: new THREE.Vector3(0.039, 0.055, 0.153) }, // #0a0e27 - Cor base azul marinho escuro
      uGradientSize: { value: 1.0 }, // Controle do tamanho do gradiente (menor = mais gradientes)
      uGradientCount: { value: 6.0 }, // Número de centros de gradiente
      uColor1Weight: { value: 1.0 }, // Peso para a cor 1 (laranja) - reduza para ter mais azul marinho
      uColor2Weight: { value: 1.0 } // Peso para a cor 2 (azul marinho) - aumente para ter mais azul marinho
    };
  }
...
class App {
 ...
 // Dentro da sua classe App ou onde gerencia o mouse
onMouseMove(ev) {
  // Isso normaliza a posição do mouse de 0 a 1
  this.mouse = {
    x: ev.clientX / window.innerWidth,
    y: 1 - (ev.clientY / window.innerHeight) // Invertido para coordenadas WebGL
  };
  ...
  // Atualiza o cursor visual (opcional)
...
window.addEventListener("scroll", () => {
  ...
  sections.forEach((section) => {
    ...
    // Ajuste o valor 200 para mudar quando o link ativa
...
document.addEventListener('DOMContentLoaded', () => {
  // 1. Configuração do Lenis (O "Motor" do Scroll)
  const lenis = new Lenis({
    duration: 1.5, // Quão suave será o scroll (maior = mais suave)
    easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)), // Curva de aceleração
  ...
  // 2. Efeito de Revelação "Staggered" (Escalonado)
  const sections = document.querySelectorAll('.section');
  
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        // Quando entra na tela: sobe, ganha cor e escala real
  ...
  sections.forEach(section => {
    // Estado inicial: levemente para baixo, menor e embaçado
    section.style.opacity = "0";
    section.style.transform = "translateY(60px) scale(0.98)";
    section.style.filter = "blur(4px)";
    section.style.transition = "all 1.2s cubic-bezier(0.22, 1, 0.36, 1)";
    observer.observe(section);
  });

  // 3. Efeito de Mouse Glow dinâmico
  document.addEventListener('mousemove', (e) => {
    const { clientX, clientY } = e;
    document.body.style.background = `radial-gradient(600px at ${clientX}px ${clientY}px, rgba(45, 108, 223, 0.15), transparent 80%), #0a192f`;
...
document.querySelectorAll('.nav-link').forEach(link => {
  link.addEventListener('mousemove', (e) => {
    ...
    // Faz a linha inclinar levemente para cima ou para baixo seguindo o mouse
    const moveY = (e.offsetY - link.offsetHeight / 2) * 0.5;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 2. BUSCA DA SEÇÃO (Clique no Menu)
  document.querySelectorAll('.nav-link').forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      if (targetSection) {
        // Rola até a seção ignorando a trava da sidebar
        lenis.scrollTo(targetSection, { offset: -80 });
      }
    });
  });

  // 3. ATUALIZA O MENU (Scroll Spy)
  lenis.on('scroll', () => {
    let current = "";
...
// No seu script de clique, use o offset para alinhar o título da seção no topo
lenis.scrollTo(targetElement, {
  offset: -50, // Ajusta para não ficar colado na borda de cima
  duration: 1.2
});